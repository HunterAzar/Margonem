...
$db_player = @mysqli_num_rows(@mysqli_query($connect,"select * from player"));
$code = @mysqli_query($connect,"select * from player order by id asc");
if(!empty($_POST['login'])){
if(!empty($_POST['haslo'])){ 
if(!empty($_POST['player'])){
$db_player_dane = @mysqli_fetch_array(@mysqli_query($connect,"select * from player where id = '".$_POST['player']."' and login = '".$_POST['login']."' and haslo = '".$_POST['haslo']."' limit 1"));
...
<select name="player">
<?php
while($wczytuje = @mysqli_fetch_array($code)){
